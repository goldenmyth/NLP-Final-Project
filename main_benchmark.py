...
def main():
    ...
    for idx, sample in enumerate(tqdm(dataset)):
        ...
        pre_txt, p_txts, que_txt = get_chat_parts(tokenizer, paragraphs, sample['question'])
        ...
        # 1. Baseline
        res_b, logits_b, attn_b = run_inference(model, tokenizer, full_prompt, max_new=config.MAX_NEW_QA)
        ...
        for strategy, transform in [("Aligned", shift_cache), ("Naive", identity_transform)]:
            ...
            res, logits, attn = run_inference(model, tokenizer, ids_que, cache_obj=cache, max_new=config.MAX_NEW_QA)
            ...
            m.update({
                ...
                "Case_ID": idx,
                "Target": answer,
                "Raw_Output": res.replace('\n', ' '),
                "Output": res,
                "Baseline_Output": res_b
            })
    ...
    df.to_csv(f"{config.SAVE_DIR}/musique_results_3b.csv", index=False)
